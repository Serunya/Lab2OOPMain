<%--
  Created by IntelliJ IDEA.
  User: gopro
  Date: 27.09.2022
  Time: 11:38
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>Валюты</title>
</head>
<body>
    <form id="rendered-form">
        <div class="rendered-form">
            <div class="formbuilder-date form-group field-date-1664267987230-0"><label for="date-1664267987230-0"
                                                                                       class="formbuilder-date-label">Дата</label><input
                    type="date" class="form-control" name="date-1664267987230-0" id="date-1664267987230-0"></div>
            <div class="formbuilder-date form-group field-date-1664267992149-0"><label for="date-1664267992149-0"
                                                                                       class="formbuilder-date-label">Дата</label><input
                    type="date" class="form-control" name="date-1664267992149-0" id="date-1664267992149-0"></div>
            <button>Получить кортировки</button>
        </div>
    </form>
</body>
</html>
